...
import { deactivateSchool } from "../../../api/axiosSchool";
import toast from "react-hot-toast";
import EditSchool from "../../forms/edit/editSchool";

export function TableSchool(props) {
  ...
  const [isOpenArray_edit, setIsOpenArray_edit] = useState(Array(row_.length).fill(false));
  ...
  return (
    ...
    <Card className="h-full w-full bg-admin-black p-5 text-white rounded-[10px] hidden sm:block">
      <table className="table min-w-full table-fixed text-left">
        ...
        {row_.filter(item => item.is_active === true)
          .map((item, index) => (
            <>
              <tr key={item.user}>
              ...
              <button className="bg-admin-blue button_tech_colorless mr-2" onClick={() => openModal_edit(index)}>Editar</button>
 ...
 <Portal open={isOpenArray_edit[index]} onClose={() => closeModal_edit(index)}> 
<EditSchool rbd={item.rbd} update={()=>props.update()} onClose={() => closeModal_edit(index)}/>
 </Portal>
