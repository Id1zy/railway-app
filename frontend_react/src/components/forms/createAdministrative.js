...
import { postSecretary } from "../../api/axiosAdministrative";

const CreateAdministrative =({signup, update, onClose}) => {
  ...
  const onSubmit = (data) => {
    if (password !== re_password) {
      ...
      postSecretary( data['first_name'], data['last_name'], data['email'],  data['password'])
      setCreateUser(true)
      onClose()
      return update()
    }
    ...
    <Card className="h-full w-full overflow-scroll bg-admin-black p-5 text-white rounded-[10px]">
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="border-b border-admin-green mb-2">
          <h2 className="text-xl3 font-bold">Registro de Secretarios</h2>
        </div>
        <div>
