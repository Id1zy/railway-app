...
import { postInspector } from "../../api/axiosInspector";

const CreateInspector =({signup, update, onClose}) => {
  ...
  const onSubmit = (data) => {
    if (password !== re_password) {
      ...
      try{
      setLoading(true);
      postInspector( data['first_name'], data['last_name'], data['email'],   data['password']);
      setLoading(false);
