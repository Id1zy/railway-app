...
import { postProfessor } from "../../api/axiosProfessor";

const CreateProfessor =({signup, update, onClose}) => {
  ...
  const onSubmit = (data) => {
    try{ 
    setLoading(true);
    postProfessor( data['first_name'], data['last_name'], data['email'],  data['rut'],  data['phone']);
    setLoading(false);
    setCreateUser(true);
    onClose();
    return update();
