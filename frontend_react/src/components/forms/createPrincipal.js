...
import { postDirector } from "../../api/axiosPrincipal";

const CreatePrincipal =({signup, update, onClose}) => {
  ...
  const onSubmit = (data) => {
    if (password !== re_password) {
      ...
      try{
      setLoading(true);
      postDirector( data['first_name'], data['last_name'], data['email'],  data['password']);
      setLoading(false);
