...
const CreateForumGuardian =({ update}) => {
  ...
  if (createForum){
    return<Navigate to={ProfessorRoutes.ForumGuardian.replace(':uid', params.uid)} />
  }
  ...
  const onSubmit = (data) => {
    ...
    if (editorRef.current){
      createForumPost( data['title'], image, data['description'],data['excerpt'], 'apoderado', section, editorRef.current.getContent())
      postNotification(1, 'Creaste el foro "'+ data['title']+'"', 'notis')
      postNotificationSection(section, 1, 'Se creo un nuevo foro "'+ data['title']+'"', 'notis')
      setCreateForum(true)
      return  update();
    }
    ...
    <Card className="h-full w-full shadow-none p-5 text-blue rounded-[10px] overflow-y-auto">
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="flex items-center">
            <Link to={ProfessorRoutes.ForumGuardian.replace(':uid', params.uid)}><ArrowLeftIcon className="h-6 w-6 text-blue hover:text-white hover:rounded-full hover:bg-blue"/></Link>
