...
import { getDirector, editDirector } from "../../../api/axiosPrincipal";
...
const EditAdmin =({id, onClose}) => {
  ...
  useEffect(() => {
    ...
    async function loadAdmin() {
      try {
        const res = await getDirector(id);
  ...
  const onSubmit = (data) => {
    if (password !== re_password) {
      ...
      editDirector(id, data['first_name'], data['last_name'], data['email'],  data['password'], )
