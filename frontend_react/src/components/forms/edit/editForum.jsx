...
import { editForum, getForumOne } from "../../../api/axiosForum";
...
const EditForum =({signup, update}) => {
  ...
  useEffect(() => {
    async function loadForum() {
      try {
        const res = await getForumOne(foro);
        if (res && res.data) {
  ...
  const onSubmit = (data) => {
    try{
        toast.success('Foro Editado')
    if (editorRef.current){
      editForum( foro, data['title'], image, data['description'],data['excerpt'], data['category'],  editorRef.current.getContent())
      setEditForum(true);
      return update()
    }
    }catch(e){
    ...
    <Card className="h-full w-full shadow-none p-5 text-blue rounded-[10px] overflow-y-auto">
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="flex items-center">
          ...
          <h2 className="text-xl3 font-bold text-blue text-center">Editar Foro</h2>
        </div>
        <div>
        ...
        <option value="off-topic">Off-Topic</option>
          ...
          <div className="flex flex-row-reverse mt-3">
            <div className="w-full md:w-auto">
              <button className='w-full md:w-auto border-2 border-blue !text-blue hover:bg-blue hover:!text-white button_tech_colorless' type="submit">Editar Foro</button>
            </div>
          </div>
        </div>
      </form>
