...
import { getStudent, editStudent } from "../../../api/axiosStudent";
...
const EditStudent =({id, onClose}) => {
  ...
  useEffect(() => {
    ...
    async function loadAdmin() {
      try {
        ...
        setValue('rutGuardian', res.data[0].rutGuardian)
  ...
  const onSubmit = (data) => {
      ...
      editStudent(id, data['first_name'], data['last_name'], data['email'],  data['rut'],data['rutGuardian'] )
