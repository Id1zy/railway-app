...
import { getInspector, editInspector } from "../../../api/axiosInspector";



const EditInspector =({id, onClose}) => {
  ...
  useEffect(() => {
    ...
    async function loadAdmin() {
      try {
        const res = await getInspector(id);
  ...
  const onSubmit = (data) => {
    if (password !== re_password) {
      ...
      editInspector(id, data['first_name'], data['last_name'], data['email'],  data['password'], )
