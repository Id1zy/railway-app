...
import { postGuardian } from "../../api/axiosGuardian";

const CreateGuardian =({signup, update, onClose}) => {
  ...
  const onSubmit = (data) => {
    try{
      setLoading(true);
      postGuardian( data['first_name'], data['last_name'], data['email'],  data['rut'],  data['phone'], data['address']);
      setLoading(false);
      ...
      toast.error('Apoderado No ha sido creado Exitosamente.');
      setLoading(false);
      onClose();
    }
  ...
  return (
    <Card className="h-full w-full overflow-scroll bg-admin-black p-5 text-white rounded-[10px]">
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        {loading ? <><h2 className="text-xl3 font-bold animate-pulse">Registrando...</h2></>:<><h2 className="text-xl3 font-bold">Registro</h2></>}
            ...
            <div className="grid grid-rows-1 gap-4 sm:grid-rows-2">
              ...
              <div className="mb-1">
                ...
                <input
                  type="text"
                  {...register('phone', {
                    required: "El Número Teléfonico es Requerido.",
                  })}
