...
const ProfileGuardian = ({  user }) => {
  const [userData, setUserData] = useState('');
  const [profileImage, setProfileImage] = useState(null);
  const irAConfiguracion = () => {
    window.location.href = '/guardian/profile/config';
};
...
<div className="grid grid-cols-1 sm:grid-cols-5 bg-white text-blue rounded-[20px] p-3 mb-3 shadow-tech-outer border-x-4 sm:border-l-4 border-blue">
  ...
  <Link to={GuardianRoutes.Settings} className='w-full md:flex-1 md:w-20 lg:w-30 bg-blue text-center text-white rounded-lg font-bold py-2 px-2 md:px-1'>
      Configuración
    </Link>
  </div>
</div>
