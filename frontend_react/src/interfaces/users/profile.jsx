// ICO
import { UserCircleIcon, Cog6ToothIcon, ArrowLeftOnRectangleIcon, ArrowLeftIcon} from '@heroicons/react/24/outline';
// React Functions
...
const Profile = ({  user }) => {
  ...
  useEffect(() => {
    ...
    <div className="flex items-center">
            <Link to={StudentRoutes.Dashboard}><ArrowLeftIcon className="h-6 w-6 text-blue hover:text-white hover:rounded-full hover:bg-blue"/></Link>
...
<div className="grid grid-cols-1 sm:grid-cols-5 bg-white text-blue rounded-[20px] p-3 mb-3 shadow-tech-outer border-x-4 sm:border-l-4 border-blue">
  ...
  <div className='flex flex-col md:flex-row justify-center md:justify-evenly'>
    ...
    <Link to={StudentRoutes.Settings} className='w-full md:flex-1 md:w-20 lg:w-30 bg-blue text-center text-white rounded-lg font-bold py-2 px-2 md:px-1'>
      Configuración
    </Link>
  </div>
</div>
