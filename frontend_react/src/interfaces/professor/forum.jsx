...
<PageUser color={`bg-white ${Style.textSchool}`} colorInput={`bg-white ${Style.textSchool}`} user={user}>
    ...
    <Card className={`w-full p-5 mb-2`}>
      <div className={`flex justify-between overflow-hidden`}>
        <div className={`flex items-center mr-5`}>
          <Link to={ProfessorRoutes.Subject}><ArrowLeftIcon className={`h-6 w-6 ${Style.textSchool} hover:text-white hover:rounded-full ${Style.hoverSchool}`} /></Link>
        </div>
        <div className={`flex-grow p-2 text-center bg-background-blue rounded`}>
          <p className={`${Style.textSchool} font-bold`}>Listado de tus Foros</p>
        </div>
        <div className={`flex items-center ml-5`}>
          <Link to={ProfessorRoutes.ForumCreate.replace(':uid', params.uid)} className={`inline-block bg-new-green hover:bg-new-hgreen button_tech_colorless`}>Crear Foro</Link>
        </div>
      </div>
    </Card>
        ...
        <Card className={`mb-2`} key={item.id}>
          <div className={`relative flex flex-col md:flex-row md:space-x-5 space-y-5 md:space-y-0 rounded-xl shadow-lg p-3 max-w-xs md:max-w-3xl mx-auto border border-white bg-white`}>
            <div className={`w-full md:w-1/3 bg-white grid place-items-center`}>
              {item.thumbnail ? <>
                <img src={`http://localhost:8000${item.thumbnail}`} alt="img" className="rounded-xl w-[400px] h-[180px]" /></> :
              <>
                <img src={"https://media-front.elmostrador.cl/2020/07/IMG_0689.jpg"} alt="img" className="rounded-xl" /></>}
            </div>
            <div className={`w-full md:w-2/3 bg-white flex flex-col space-y-2 p-3`}>
              <div className={`flex justify-between item-center`}>
                ...
                <div className={`inline-flex gap-2`}>
                  <div className={`bg-orange px-3 py-1 rounded-full text-xs font-medium `}>
                  ...
                  : 
                  <> 
                    <div className={`bg-admin-red px-3 py-1 rounded-full text-xs font-medium  `}>
                      <p className={`text-white`}>{item.status}</p>
                    </div>
                  </>
                  }
              ...
              <div className={`flex justify-end mt-4 gap-2 `}>
                <Link to={ProfessorRoutes.ForumView.replace(':uid', params.uid).replace(':forum', item.id)} className={`${Style.colorSchool} button_tech_colorless`}>Ingresar</Link>
          ...
          <div className={`w-full max-w-md transform overflow-hidden rounded-2xl bg-white ${Style.textSchool} p-6 text-left align-middle shadow-xl transition-all`}>
            ...
            <div className={`flex justify-center`}>
              <div className={`inline-flex  items-center`}>
                <button onClick={()=>closeModal_2(index)} className={`border-2 ${Style.borderSchool} !${Style.textSchool}  hover:bg-admin-blue hover:!text-white button_tech_colorless mr-2`}>Cancelar</button>
                {item.status === 'publicado' ? <>
                  { loading ?
  ...
  <Link to={ProfessorRoutes.ForumView.replace(':uid', params.uid).replace(':forum', item.id)} className={`font-bold text-new-blue text-sm hover:text-new-hblue`}>{item.title}</Link>
  </div>
  </>))}
