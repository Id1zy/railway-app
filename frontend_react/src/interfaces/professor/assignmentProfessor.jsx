...
import { createAssignment } from "../../api/axiosAssignment";
import { postNotificationSection } from "../../api/axiosNotification";
...
const CreateAssignment = ({  user }) => {
  ...
  if (editorRef.current){
    try{
        createAssignment(params.uid, data['title'], editorRef.current.getContent(), startDate);
        postNotificationSection(params.uid, 1, 'Trabajo Asignado.', data['title']+' - Fecha Límite:'+startDate)
    ...
    <Card className={`w-auto md:min-w-screen shadow-none p-5 ${Style.textSchool} rounded-[10px] transition-all`}>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className={`border-b-2 ${Style.borderSchool} mb-2`}>
          <h2 className={`text-xl3 font-bold ${Style.textSchool} text-center`}>Crear Trabajo</h2>
        </div>
        <div>
          ...
          <div className={`grid grid-cols-1 gap-4  mb-1`}>
            ...
            <div className={`grid grid-cols-1 gap-4 w-full mb-1`}>
              <div>
                ...
                <DatePicker selected={startDate} onChange={(date) => setStartDate(date)} showTimeSelect className={`${styles.true} z-50 !w-full`}/>
              </div>
            </div>
          </div>
        ...
        <div className={`flex flex-row-reverse mt-3`}>
          <div className={`w-full md:w-auto`}>
            <button className={`w-full md:w-auto border-2 ${Style.borderSchool} !${Style.textSchool} hover:${Style.hoverSchool} hover:!text-white button_tech_colorless`} type="submit">Crear</button>
          </div>
        </div>
    </form>
  </Card>
