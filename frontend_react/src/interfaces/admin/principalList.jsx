...
import { TablePrincipal } from "../../components/admin/tables/tablePrincipal";
import { TablePrincipalDeactivate } from "../../components/admin/tables/tablePrincipalDeactivate";
import CreatePrincipal from "../../components/forms/createPrincipal";
import { listDirector, uploadDirector, downloadDirector } from "../../api/axiosPrincipal";
...
import toast from "react-hot-toast";
  ...
  const handleDownload = async () =>{
    try{
      const res = await downloadDirector();
      if(res.status == 200){
        toast.success('Ha comenzado la descarga.')
        return saveAs(res.data, 'ReporteDirector.xls')
      }
      else{
...
const postFile = (e) =>{
  ...
  useEffect(() => {
    async function loadUsers() {
      try {
        const res = await listDirector();
        if (res && res.data) {
          setUsers(res.data); 

        }
      } catch (error) {
        toast.error('Error al cargar al Director.'); 
      }
    }
    loadUsers();
    return ()=>setUpdate(0);
    ...
    const data = [
        {
          ...
          desc:
          <>
             <TablePrincipal head={HEAD} row={ROW} update={()=>setUpdate(update+1)} />
          </>
        ...
        },
        {
          ...
          desc:
          <>
             <TablePrincipalDeactivate head={HEAD} row={ROW} update={()=>setUpdate(update+1)} />
          </>
  ...
  />
      ...
      <button onClick={(e)=>postFile(e)}
       className='w-full font-bold md:flex-1 md:w-40 mb-2 md:mb-0 sm:mr-2 bg-admin-green hover:bg-new-dark-green text-white rounded-lg py-2 px-4'>
            ...
            <CreatePrincipal update={()=>setUpdate(update+1)} onClose={() => closeModal()}/>
          </Portal>
