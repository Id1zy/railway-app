...
import { listUTP, downloadUTP, uploadUTP } from "../../api/axiosUTP";
import { TabsCustomAnimation } from "../../components/tabs";
import { TableUTP } from "../../components/admin/tables/tableUTP";
import { TableUTPDeactivate } from "../../components/admin/tables/tablaUTPDeactivate";
import CreateUTP from "../../components/forms/createUTP";
...
const handleDownload = async () =>{
    try{
      const res = await downloadUTP();
      if(res.status == 200){
        toast.success('Ha comenzado la descarga.')
        return saveAs(res.data, 'ReporteUTP.xls')
      }
      else{
...
const postFile = (e) =>{
  ...
  useEffect(() => {
    async function loadUsers() {
      try {
        const res = await listUTP();
        if (res && res.data) {
          setUsers(res.data); 
        }
      } catch (error) {
        toast.error('Error al cargar a los UTPs.'); 
      }
    }
    loadUsers();
    return setUpdate(0);
    ...
    const data = [
        {
          ...
          desc:
          <>
             <TableUTP head={HEAD} row={ROW} update={()=>setUpdate(update+1)} />
          </>
        },
        {
          label: "Desactivado",
          ...
          desc:
          <>
             <TableUTPDeactivate head={HEAD} row={ROW} update={()=>setUpdate(update+1)} />
          </>
        },
      ];
    ...
    return(
        <PageUser color={'bg-admin-black text-white'} colorInput={'bg-admin-input text-white placeholder-white'}>
          <SideBar  color={'bg-admin-black'} colorSecond={'admin-green'} useRol={'Administrador'} mapeo={index_admin}>
            {/* Parte Central */}
            <div className="w-full md:w-4/5 bg-admin-background overflow-y-auto  p-4 h-full">    
      ...
      <button onClick={(e)=>postFile(e)}
       className='w-full font-bold md:flex-1 md:w-40 mb-2 md:mb-0 sm:mr-2 bg-admin-green hover:bg-new-dark-green text-white rounded-lg py-2 px-4'>
          ...
          <Portal open={isOpen} onClose={() => closeModal()}>
            <CreateUTP update={()=>setUpdate(update+1)} onClose={() => closeModal()}/>
          </Portal>
