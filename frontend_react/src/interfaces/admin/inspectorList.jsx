...
import { listInspector, uploadInspector, downloadInspector } from "../../api/axiosInspector";
import { TabsCustomAnimation } from "../../components/tabs";
import { TableInspector } from "../../components/admin/tables/tableInspector";
import { TableInspectorDeactivate } from "../../components/admin/tables/tableInspectorDeactivate";
import CreateInspector from "../../components/forms/createInspector";
...
import toast from "react-hot-toast";
  ...
  const handleDownload = async () =>{
    try{
      const res = await downloadInspector();
      if(res.status == 200){
        toast.success('Ha comenzado la descarga.')
        return saveAs(res.data, 'ReportesInspectores.xls')
      }
      else{
    ...
    const data = [
        {
          ...
          desc:
          <>
             <TableInspector head={HEAD} row={ROW}  update={()=>setUpdate(update+1)}/>
          </>
        ...
        },
        {
          ...
          desc:
          <>
             <TableInspectorDeactivate head={HEAD} row={ROW}  update={()=>setUpdate(update+1)}/>
          </>
  ...
  />
      ...
      <button onClick={(e)=>postFile(e)}
       className='w-full font-bold md:flex-1 md:w-40 mb-2 md:mb-0 sm:mr-2 bg-admin-green hover:bg-new-dark-green text-white rounded-lg py-2 px-4'>
            ...
            <CreateInspector update={()=>setUpdate(update+1)} onClose={() => closeModal()}/>
          </Portal>
