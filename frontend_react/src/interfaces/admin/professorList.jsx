...
import { TableProfessor } from "../../components/admin/tables/tableProfessor"; 
import { TableProfessorDeactivate } from "../../components/admin/tables/tableProfessorDeactivate";
import CreateProfessor from "../../components/forms/createProfessor";  
import { listProfessor, uploadProfessor, downloadProfessor} from "../../api/axiosProfessor";  
...
const handleDownload = async () =>{
    try{
      const res = await downloadProfessor();
      if(res.status == 200){
        toast.success('Ha comenzado la descarga.')
        return saveAs(res.data, 'ReporteProfesores.xls')
      }else{
...
const ProfessorList = () => {  
  ...
  useEffect(() => {
    async function loadUsers() {
      try {
        const res = await listProfessor();  
        if (res && res.data) {
          setUsers(res.data);
        }
      } catch (error) {
        toast.error('Error al cargar a los profesores.'); 
      }
    }
    loadUsers();
    return setUpdate(0);
  ...
  const HEAD = ["Rut", "Nombre", "Colegios", "Correo Electrónico", "Teléfono", "Funciones"];
  const ROW = users
    ...
    const data = [
        {
          ...
          desc:
          <>
          <TableProfessor head={HEAD} row={ROW} update={()=>setUpdate(update+1)} /> 
          </>
        ...
        },
        {
          ...
          desc:
          <>
          <TableProfessorDeactivate head={HEAD} row={ROW} update={()=>setUpdate(update+1)}/> 
          </>
      ...
      <button onClick={(e)=>postFile(e)}
       className='w-full font-bold md:flex-1 md:w-40 mb-2 md:mb-0 sm:mr-2 bg-admin-green hover:bg-new-dark-green text-white rounded-lg py-2 px-4'>
          ...
          <Portal open={isOpen} onClose={() => closeModal()}>
            <CreateProfessor update={()=>setUpdate(update+1)} onClose={() => closeModal()}/>  
          </Portal>
   </PageUser>
    )
}
